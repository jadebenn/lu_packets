...
pub fn derive_bit_variant_tests(input: TokenStream) -> TokenStream {
    variant_tests::derive(
        input,
        quote!(::endio_bit::BEBitReader::new(&mut bin)),
        quote!(::endio_bit::BEBitWriter::new(&mut out)),
    )
}
...
pub fn derive_replica_variant_tests(input: TokenStream) -> TokenStream {
    variant_tests::derive(
        input,
        quote!(crate::raknet::client::replica::DummyContext { inner: &mut bin }),
        quote!(&mut out),
    )
}
