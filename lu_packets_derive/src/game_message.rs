...
use quote::quote;
use syn::{
    Data, DeriveInput, Field, Fields, Meta, NestedMeta, Type, parse_macro_input, parse_quote,
};
...
pub fn derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    des_impl_generics
        .params
        .push(parse_quote!(__READER: ::std::io::Read));
    ...
    ser_impl_generics
        .params
        .push(parse_quote!(__WRITER: ::std::io::Write));
...
fn gen_deser_code(fields: &Fields) -> TokenStream {
    ...
    for f in &fields.named {
        ...
        let val = if is_bool {
            ...
            let parse = quote! { crate::world::gm::GmParam::deserialize(reader)? };
            match default {
                None => quote! { #parse },
                Some(default) => quote! {
                    if reader.read_bit()? {
...
fn gen_ser_code(fields: &Fields) -> TokenStream {
    ...
    for f in &fields.named {
        ...
        let write = if is_bool {
            ...
            let write_field =
                quote! { crate::world::gm::GmParam::serialize(&self.#ident, writer)? };
            match default {
...
fn get_gm_default(input: &Field) -> Option<NestedMeta> {
    for attr in &input.attrs {
        ...
        let nested_meta = list
            .nested
            .first()
            .expect("default attribute should have exactly one argument");
